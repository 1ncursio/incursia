...
router.post('/mail', async(req, res, next) => {
    ...
    if (!user) {
        return res.status(403).json({
            success: false,
            message: '가입되지 않았거나 이미 인증이 완료된 계정입니다.',
        });
    }
...
router.patch('/validation', testMiddleware, async(req, res, next) => {
    try {
        const user = await User.findOne({
            where: { token: req.body.token, status: 'pending' },
        });
        if (!user) {
            return res.status(403).json({
                success: false,
                message: '인증이 완료되었거나, 유효시간이 지났습니다.',
            });
        }
...
router.patch('/introduction', isLoggedIn, async(req, res, next) => {
    try {
        ...
        res
            .status(200)
            .json({ success: true, introduction: req.body.introduction });
    } catch (error) {
        console.error(error);
        next(error);
    }
